...
function createIcons() {
  ...
  const settings = createSettingsIcon();
  ...
  settings.setAttribute(
    "style",
    `transform translate(${box.height}, ${center.x})`
  );
...
/** returns center x y coords of window */
function getCenter() {
  return {
...
function createSettingsIcon(parent: SVGElement) {
  ...
  group.setAttribute("id", "settings");
  const x = coords.width / 2 - X_OFFSET;
  const y = coords.bottom - Y_OFFSET;
...
function createTimeSigIcon(parent: SVGElement) {
  ...
  group.setAttribute("stroke-miterlimit", "2");
  group.setAttributeNS(null, "transform", `translate(${x},${y})`);

  group.setAttribute("id", "time-sig");
